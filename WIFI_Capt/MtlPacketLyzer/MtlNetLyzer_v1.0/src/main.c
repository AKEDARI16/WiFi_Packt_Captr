...
extern void *connect_capture_thread(void *);
extern void *connect_parse_thread(void *);
extern void *connect_thread_implement(char *, char *interface, pcap_t *);

extern void *packet_capture_thread(void *);
extern void *packet_parse_thread(void *);
extern void *capture_thread_implement(char *, char *interface, pcap_t *);

extern void *scan_capture_thread(void *);
extern void *scan_parse_thread(void *);
extern void *scan_thread_implement(char *, char *interface, pcap_t *);

extern void *handshake_capture_thread(void *);
extern void *handshake_parse_thread(void *);
extern void *handshake_thread_implement(char *, char *interface, pcap_t *);

extern void *handshake_implement(char *, char *interface, pcap_t *);
...
int main(int argc, char *argv[]) {
	...
	while ((opt = getopt(argc, argv, "c:p:hs:w")) != -1) {
		...
		case 'c':
			...
			connect_thread_implement(filter, interface, handle);
		...
		case 'p':
			...
			capture_thread_implement(filter, interface, handle);
			break;
		case 's':
			/* Assign corresponding functions to function pointers */
			...
			scan_thread_implement(filter_exp, interface, handle);
			break;
		case 'w':
			/*bfill_fptr = &handshake_capture_thread;
			bparse_fptr = &handshake_parse_thread;*/
			strcpy(filter_exp1, "ether proto 0x888e");
			//char *filter_exp1 = "ether proto 0x888e"; // Filter expression for EAPOL frames
